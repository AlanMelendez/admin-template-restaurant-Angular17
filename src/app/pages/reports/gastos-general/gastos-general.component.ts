...
@Component({
  selector: 'app-gastos-general',
  templateUrl: './gastos-general.component.html',
  styleUrl: './gastos-general.component.css'
})
export class GastosGeneralComponent {
    ...
    for (let i = 1; i <= 100; i++) {
      ...
      this.displayedColumns.forEach((column) => {
        switch (column) {
          ...
          //   break;
          case 'TipoGasto':
            rowData[column] = `TipoGasto  ${i}`;
            break;
          case 'FechaHora':
            rowData[column] = new Date().toLocaleString();
            break;
          case 'Provedor':
            rowData[column] = `Provedor ${i}`;
            break;
          case 'Responsable':
